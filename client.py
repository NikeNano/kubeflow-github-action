...
def _add_generated_apis(target_struct, api_module, api_client):
    ...
    for member_name in dir(api_module.models):
        if not member_name[0].islower():
            setattr(models_struct, member_name, getattr(
                api_module.models, member_name))
...
class Client(object):
    ...
    def _load_config(self, host, client_id, namespace, other_client_id, other_client_secret):
        ...
        if config.host:
            config.host = config.host + '/' + \
                Client.KUBE_PROXY_PATH.format(namespace)
        return config
    ...
    def _is_iap_host(self, host, client_id):
        if host and client_id:
            if re.match(r'\S+.endpoints.\S+.cloud.goog/{0,1}$', host):
                warnings.warn(
                    'Suffix /pipeline is not ignorable for IAP host.')
    ...
    def create_run_from_pipeline_package(self, pipeline_file: str, arguments: Mapping[str, str], run_name=None, experiment_name=None, namespace=None):
        ...
        run_info = self.run_pipeline(
            experiment.id, run_name, pipeline_file, arguments, namespace=namespace)
